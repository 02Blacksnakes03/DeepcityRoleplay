	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('CanLip-Cactus:canPickUp', function(canPickUp)
					...
					--	Citizen.Wait(2000)
						...
						TriggerServerEvent('CanLip-Cactus:pickedUpwoods')
					else
...
RegisterNetEvent('CanLip-Cactus:CheackingOK')
AddEventHandler('CanLip-Cactus:CheackingOK', function()
	itemFound = true
end)


RegisterNetEvent('CanLip-Cactus:CheackingSellOK')
AddEventHandler('CanLip-Cactus:CheackingSellOK', function(itemValue)
	itemFoundSell = true
	itemValue = 1
end)

RegisterNetEvent('CanLip-Cactus:CheackingNO')
AddEventHandler('CanLip-Cactus:CheackingNo', function()
	itemFound = false
end)
  ...
  if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, Config.CircleZones.woodCut.coords.x, Config.CircleZones.woodCut.coords.y, Config.CircleZones.woodCut.coords.z, true) < 4) then
		local playerPed = PlayerPedId()
          ...
          if distance < 2 then
            ...
            TriggerServerEvent('CanLip-Cactus:CheackingPack')
            end
            ...
            if itemFound == true  then
            	...
            	StatusReady = false
            	procent(50)
             	TriggerServerEvent('CanLip-Cactus:pack')
            	ClearPedTasks(playerPed)
            	Citizen.Wait(1000)
            	StatusReady = true
            	else
  ...
  if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, Config.CircleZones.woodSell.coords.x, Config.CircleZones.woodSell.coords.y, Config.CircleZones.woodSell.coords.z, true) < 4) then
		local playerPed = PlayerPedId()
          ...
          if distance < 2 then
            ...
            if IsControlJustReleased(1,  38) and StatusReady == true  then
            itemFoundSell = false
            TriggerServerEvent('CanLip-Cactus:SellCheak')
             FreezeEntityPosition(playerPed, true)
            Citizen.Wait(500)
             FreezeEntityPosition(playerPed, false)
            if itemFoundSell == true  then
            	...
            	procent(150)
             	TriggerServerEvent('CanLip-Cactus:Sellpack')
            	ClearPedTasks(playerPed)
            	Citizen.Wait(300)
            	StatusReady = true
            	else
...
function Spawnwoods()
	while spawnedshellfish < 70 do
		...
		ESX.Game.SpawnLocalObject('prop_cactus_02', woodCoords, function(obj)
